...
for random_seed in random_seeds:
	## build attack target
	# batch_target, ind_A = random_target(dataset=dataset_predcorrect,min_a=min_a,batch_id=batch_id,batch_size=batch_size,classifier_name=classifier_name)
	# batch_target, ind_A = corre_target(dataset=dataset_predcorrect,batch_id=batch_id,batch_size=batch_size,classifier_name=classifier_name)
...
# adv_eps0_norm = np.sqrt(np.sum(np.square(adv_eps[0] - batch_x), axis=1))
# adv_eps1_norm = np.sqrt(np.sum(np.square(adv_eps[1] - batch_x), axis=1))
# adv_eps2_norm = np.sqrt(np.sum(np.square(adv_eps[2] - batch_x), axis=1))
