...
from sklearn.metrics import f1_score , precision_score , recall_score , accuracy_score , jaccard_similarity_score , \
	hamming_loss , coverage_error , label_ranking_loss , label_ranking_average_precision_score
...
def one_grad(x,i):
	...
	# grad_nl = -1 * grad * cp_one_label/K.exp(cp_one_label * cp_one_out)
	...
	# grad_nl = -1 * grad * cp_one_label / K.log(K.exp(0.1 ) + K.exp(K.exp(cp_one_label * cp_one_out)))
	# grad_nl = -1 * grad * cp_one_label
...
def fit(dataset , folds , classifier):
	...
	##soft_lambda = [0.015, 0.01 , 0.005, 0.002, 0.001 ,0.0001, 0]
	loss_lambda = 0.002
	tradeAB = 0.5
	...
	for j in range(1):
		...
		if classifier == 'svm':
			...
			# model.compile(optimizer=optimizer , loss="hinge" , metrics=acc_svm)
		else:
		...
		# model.save('model_trans/' + dataset + 'softlambda' + str(loss_lambda) + '.h5')
		...
		# singular = np.linalg.svd(weights , full_matrices=False , compute_uv=False)
		...
		with open('performance/' + dataset + 'per' + method + 'lambda' + str(loss_lambda)  + "_tradeab" + str(tradeAB) + '.txt' , 'w') as f:
			...
			f.write("%s\n" % macrof1)
			# exact = 'exact_match: ' + str(exact_match_mean) + "; " + str(exact_match_std)
			# f.write("%s\n" % exact)
			# accuracy = 'accuracy: ' + str(accuracy_mean) + "; " + str(accuracy_std)
			# f.write("%s\n" % accuracy)
			# coverage = 'coverage: ' + str(coverage_mean) + "; " + str(coverage_std)
			# f.write("%s\n" % coverage)
			# ranking = 'ranking: ' + str(ranking_loss_mean) + "; " + str(ranking_loss_std)
			# f.write("%s\n" % ranking)
			# hamming = 'hamming: ' + str(hamming_mean) + "; " + str(hamming_std)
			# f.write("%s\n" % hamming)
			# precision = 'precision: ' + str(average_precision_mean) + "; " + str(average_precision_std)
			# f.write("%s\n" % precision)
